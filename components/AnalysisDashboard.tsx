  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
            <h1 className="text-2xl font-bold text-foreground">Pre-Call Dossier</h1>
            <p className="text-muted-foreground">Candidate: {data.firstName} {data.lastName} ({data.companyName})</p>
        </div>
        <button
            onClick={onReset}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-card border border-border rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Fit Score</h3>
            <div className="relative pt-2">
              <div className="flex items-end justify-between mb-2">
                <span className="text-5xl font-bold text-primary">{analysis.estimatedFitScore}</span>
          ...
          <div className="bg-card border border-border rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Psychological Profile</h3>
            <p className="text-foreground leading-relaxed italic border-l-4 border-primary/30 pl-4 text-sm">
                "{analysis.clientPsychology}"
            </p>
          </div>

          <div className="bg-card border border-border rounded-xl shadow-sm p-6">
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4">Key Signals</h3>

            <div className="space-y-4">
                <div>
                    <span className="text-xs font-bold text-destructive flex items-center gap-1 mb-2">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                        RED FLAGS
                    </span>
                    <ul className="space-y-2">
                        {analysis.redFlags.length === 0 ? <li className="text-sm text-muted-foreground">None detected.</li> :
                         analysis.redFlags.map((flag, i) => (
                            <li key={i} className="text-sm text-foreground flex items-start gap-2">
                                <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-destructive/30 shrink-0"></span>
                                {flag}
                            </li>
                        ))}
                    </ul>
                ...
                <div>
                    <span className="text-xs font-bold text-terminal-green flex items-center gap-1 mb-2">
                        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                        GREEN FLAGS
                    </span>
                    <ul className="space-y-2">
                        {analysis.greenFlags.length === 0 ? <li className="text-sm text-muted-foreground">None detected.</li> :
                        analysis.greenFlags.map((flag, i) => (
                            <li key={i} className="text-sm text-foreground flex items-start gap-2">
                                <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-terminal-green/30 shrink-0"></span>
                                {flag}
                            </li>
                        ))}
                    </ul>
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-[var(--terminal-yellow)]/10 rounded-xl shadow-sm border border-[var(--terminal-yellow)]/30 p-6">
                    <h3 className="text-sm font-bold text-[var(--terminal-yellow)] uppercase tracking-wider mb-3 flex items-center gap-2">
                         <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        Operational Gap
                    </h3>
                    <p className="text-[var(--terminal-yellow)] leading-relaxed text-sm">
                ...
                <div className="space-y-4">
                    {analysis.strategicQuestions.map((q, i) => (
                        <div key={i} className="flex gap-4 p-4 rounded-lg bg-secondary border border-border transition-colors hover:bg-secondary/70">
                            ...
                            <p className="text-foreground font-medium pt-1">{q}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="border-t border-border pt-6">
                 <details className="group">
                    <summary className="flex cursor-pointer items-center text-sm font-medium text-muted-foreground hover:text-foreground">
                         View Raw Submission
                    ...
                    <div className="mt-4 bg-secondary rounded-lg p-4 text-xs font-mono text-muted-foreground whitespace-pre-wrap max-h-60 overflow-y-auto">
